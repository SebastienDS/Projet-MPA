...
<table class="caracteristiques">
    <tr>
        ...
        <th>
            <a href="<?= SCRIPT_NAME ?>/bank.php/client/compte/<?= $numeroCompte ?>/transaction/<?= $siren ?>/<?= $date ?>/?colSorted=datetr&sortDirection=<?= ($colSorted !== 'datetr') ? 'ASC' : (($sortDirection === 'ASC') ? 'DESC' : 'ASC') ?>&<?= $otherParams ?>"> Date traitement </a>
        </th>
        <th>
            Devise
        </th>
        <th>
            <a href="<?= SCRIPT_NAME ?>/bank.php/client/compte/<?= $numeroCompte ?>/transaction/<?= $siren ?>/<?= $date ?>/?colSorted=moyenPay&sortDirection=<?= ($colSorted !== 'moyenPay') ? 'ASC' : (($sortDirection === 'ASC') ? 'DESC' : 'ASC') ?>&<?= $otherParams ?>"> Moyen de paiement </a>
        </th>
        <th>
            <a href="<?= SCRIPT_NAME ?>/bank.php/client/compte/<?= $numeroCompte ?>/transaction/<?= $siren ?>/<?= $date ?>/?colSorted=montant&sortDirection=<?= ($colSorted !== 'montant') ? 'ASC' : (($sortDirection === 'ASC') ? 'DESC' : 'ASC') ?>&<?= $otherParams ?>"> Montant </a>
        </th>
        <th>
            Sens + ou -
        </th>
...
<div class="footer-fixed space-between center-y">
    ...
    <div class="DL-btns space-around">
        <form action="<?= SCRIPT_NAME ?>/bank.php/client/download/pdf/compte/<?= $numeroCompte ?>/transaction/<?= $siren ?>/<?= $date ?>?<?= http_build_query($_GET); ?>" method="POST">
            <button class="btn">PDF</button>
        </form>
        <form action="<?= SCRIPT_NAME ?>/bank.php/client/download/xls/compte/<?= $numeroCompte ?>/transaction/<?= $siren ?>/<?= $date ?>?<?= http_build_query($_GET); ?>" method="POST">
            <button class="btn">XLS</button>
        </form>
        <form action="<?= SCRIPT_NAME ?>/bank.php/client/download/csv/compte/<?= $numeroCompte ?>/transaction/<?= $siren ?>/<?= $date ?>?<?= http_build_query($_GET); ?>" method="POST">
