...
<table class="caracteristiques">
    <tr>
        <th>
            <a href="<?= SCRIPT_NAME ?>/bank.php/productOwner/client/<?= $idClient ?>/compte/<?= $numeroCompte ?>/transaction/<?= $siren ?>/<?= $date ?>/?colSorted=NSIREN&sortDirection=<?= ($colSorted !== 'NSIREN') ? 'ASC' : (($sortDirection === 'ASC') ? 'DESC' : 'ASC') ?>&<?= $otherParams ?>"> Numéro de SIREN </a>
        </th>
        <th>
